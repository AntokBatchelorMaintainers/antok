...
#//      ROOT_CPP_STD           - C++ standard used building ROOT
...
set(ROOT_CPP_STD           NOTFOUND)
set(ROOT_VERSION           NOTFOUND)
set(ROOT_PYTHON_VERSION    NOTFOUND)
...
if(NOT ROOT_CONFIG_EXECUTABLE)
	...
	execute_process(COMMAND ${ROOT_CONFIG_EXECUTABLE} --cflags
		OUTPUT_VARIABLE ROOT_CPP_STD
		OUTPUT_STRIP_TRAILING_WHITESPACE)
	if(ROOT_CPP_STD)
		string(REGEX REPLACE ".*-std=c\\+\\+([0-9xyz]+).*" "\\1"
			ROOT_CPP_STD "${ROOT_CPP_STD}")
		if(   (ROOT_CPP_STD STREQUAL "0x")
		   OR (ROOT_CPP_STD STREQUAL "1x"))
			set(ROOT_CPP_STD "11")
		elseif(ROOT_CPP_STD STREQUAL "1y")
			set(ROOT_CPP_STD "14")
		elseif(ROOT_CPP_STD STREQUAL "1z")
			set(ROOT_CPP_STD "17")
		endif()
	endif()
	...
	string(REGEX REPLACE "^([0-9]+)\\.[0-9]+\\.[0-9]+.*" "\\1"
		ROOT_PYTHON_MAJOR_VERSION "${ROOT_PYTHON_VERSION}")
	string(REGEX REPLACE "^[0-9]+\\.([0-9]+)\\.[0-9]+.*" "\\1"
		ROOT_PYTHON_MINOR_VERSION "${ROOT_PYTHON_VERSION}")

	if(ROOT_PYTHON_VERSION EQUAL NOTFOUND)
		set(ROOT_FOUND FALSE)
		set(ROOT_ERROR_REASON "${ROOT_ERROR_REASON} Cannot find find python version used by ROOT.")
	endif()
...
if(ROOT_LIBRARIES)
	...
	if(ROOT_FIND_COMPONENTS)
		if(ROOT_VERSION VERSION_LESS 6.22)
			string(REGEX REPLACE "ROOTTPython" "PyROOT" ROOT_FIND_COMPONENTS "${ROOT_FIND_COMPONENTS}")
		endif()
...
find_package_handle_standard_args(ROOT
	...
	REQUIRED_VARS ROOT_CONFIG_EXECUTABLE ROOTSYS ROOT_TARGET ROOT_CC ROOT_CPP ROOT_CPP_STD ROOT_VERSION ROOT_MAJOR_VERSION ROOT_MINOR_VERSION ROOT_PATCH_VERSION ROOT_BIN_DIR ROOT_INCLUDE_DIR ROOT_LIBRARY_DIR ROOT_LIBRARIES ROOT_AUX_LIBRARIES ROOT_LIBS _DICT_EXECUTABLES_FOUND
...
if(ROOT_FOUND AND NOT ROOT_FIND_QUIETLY)
	...
	message(STATUS "ROOT was compiled with the standard c++${ROOT_CPP_STD}.")
endif()
